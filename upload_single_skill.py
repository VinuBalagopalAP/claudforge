#!/usr/bin/env python3
"""
ClaudForge - Single Skill Uploader
----------------------------
1. Zips a single skill folder
2. Opens claude.ai/customize/skills in Chrome via Playwright
3. Uploads the zip through the UI
"""
...
def main():
    parser = argparse.ArgumentParser(description="Zip and upload a single Claude skill.")
    parser.add_argument("--path", required=True, help="Path to the skill folder")
    parser.add_argument("--headless", action="store_true", help="Run browser headlessly")
    parser.add_argument("--keep-zip", action="store_true", help="Keep generated zip file after upload")
    parser.add_argument("--zip-dir", default=None, help="Where to write zip file (default: <path>/../_zips)")
    ...
    zip_dir = Path(args.zip_dir) if args.zip_dir else folder.parent / "_zips"
    zip_dir.mkdir(parents=True, exist_ok=True)
    zp = zip_folder(folder, zip_dir)

    print(f"\n📁  Skill Folder: '{folder.name}'")
    print(f"🗜   Zipped to '{zp.name}'")
    ...
    with sync_playwright() as p:
        ...
        print(f"⬆️   Uploading {folder.name} … ", end="", flush=True)
